...
async function fetchAndParse(url) {
    try {
        ...
        // --- 统一扫描所有可能的视频卡片入口 ---
        // 涵盖了 1. search-doujin-videos (里番) 2. video-item-container (本日排行) 3. home-rows-videos (旧版)
        $('a[href*="/watch?v="]').each((i, el) => {
            ...
            // --- 多策略提取标题 ---
            // 按照你提供的源码：优先找 a 下面的 .title，然后找旧版的类名，最后找 img 的 alt
            let title = $a.find('.title').text().trim() || 
                        $a.find('.home-rows-videos-title').text().trim() || 
                        $a.find('.card-mobile-title').text().trim() || 
                        $a.attr('title') || 
                        $a.find('img').attr('alt') || "";
            ...
            // 排除背景占位图
            if (poster && poster.indexOf('background.jpg') !== -1) poster = "";
            ...
            // --- 提取附加信息 (时长、作者等) ---
            const duration = $a.find('.duration, .card-mobile-duration').first().text().trim();
            const author = $a.find('.subtitle, .card-mobile-user').text().trim();

            // 只要有链接和标题，就认为是一个有效的视频项
            if (title && link) {
                items.push({
                    id: link,
                    type: "url", 
